...
def run():
    ...
    param_dict = dict_of(max_episodes, max_steps, discount_factor, p_learning_rate, v_learning_rate, n_v_iter,
                         v_update_interval, render, baseline, actor_critic, v_layers)
    ...
    with tf.Session() as sess:
        ...
        for episode in range(max_episodes):
            ...
            for t, transition in enumerate(episode_transitions):
                ...
                if actor_critic:
                    R_t = transition.reward + (1 - transition.done) * discount_factor * sess.run(value_net.value, {
                        value_net.state: transition.next_state})  # R + v'(S')
                else:
                ...
                feed_dict = {policy.state: transition.state, policy.R_t: A_t if baseline else R_t,
